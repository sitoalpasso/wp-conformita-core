...
l'identità della cartella provata, cioè il suo percorso vero più il suo indirizzo, e
un'impronta di proprietari e permessi delle cartelle, dalla cartella dell'esca fino a quella
dei caricamenti, presa nel momento della prova. Un esito si legge solo se tutte e due sono
quelle di adesso; altrimenti vale come mai misurato. Righe C-187, C-189, C-190 e C-191.

Accanto ai file di regole resta un'esca fissa, `prova-accesso-diretto.txt`, che nasce con loro
e, come loro, se manca si riscrive e fa dimenticare gli esiti. Non è il file che la verifica
chiede al server: quello nasce a ogni verifica con un nome nuovo e si toglie finita la
richiesta. Riga C-193.
...
| Un documento arriva con un nome che comincia come quello delle esche, in maiuscole o minuscole | si rifiuta con un codice suo, all'ingresso e nella guardia: tutti quei nomi sono riservati alle esche, così un documento non può mai essere scambiato per un'esca né prenderne il posto | sì |
...
| C-189 | fatto | Con una maschera dei permessi stretta e un server senza regole per i `.pdf` che serve solo i file leggibili da tutti, il deposito si rifiuta: l'esca ha i permessi che WordPress darebbe al documento, e un'esca vecchia rimasta con permessi stretti non conta, perché non è lei che si chiede. Cambiati i permessi della sottocartella o della cartella protetta, l'esito misurato prima non vale, senza richieste, e la verifica si rifà |
...
risposta già data. Dal G43 al G46, e di nuovo dal G38 al G40 dopo le correzioni del nono giro,
ognuno con la suite intera: il G43, il G44 e il G45 fanno diventare rossa la sola loro riga; il
G38 la C-187, il G39 la C-187 e la C-183, il G40 le due prove della C-187; il G46, che lascia il
segno del deposito in corso dopo il primo, fa rifiutare ogni deposito successivo e manda in
...
verifica continua non ha; il controllo è di tre righe e si legge a occhio. Dal G55 al G57, e di
nuovo il G50 e il G51 dopo le correzioni dell'undicesimo giro, ognuno con la suite intera: il
G50 fa diventare rosse la seconda prova della C-189 e le due della C-191, il G51 la prova della
...
equivalente distinto: sul codice nuovo coincide con il G58. Dopo le correzioni del
tredicesimo giro, con la suite intera, dal G60 al G66, più il G47 e il G54 rifatti sul codice
nuovo, dove l'esca non si ricrea più al posto di quella vecchia ma nasce con un nome suo: il
G60, che torna al nome fisso, fa diventare rosse diciassette prove, fra cui la C-193 con il
deposito che riusciva, e le altre perché riconoscono l'esca dalla forma del nome; il G66, lo
stesso nome fisso con un parametro casuale nell'indirizzo, cioè la correzione che la revisione
aveva detto insufficiente, ventiquattro, anche lì con la C-193 e il deposito che riusciva; il
G61, che lascia l'esca dopo la richiesta, la C-193, la C-192, la C-181, la C-180 e la C-183; il
G62 e il G63, che riservano solo il nome esatto o solo le minuscole, la sola C-181; il G65, che
...
controlla a occhio; una prova non lo può fare. **Gli altri undici non
...
| G43 | Tolto il controllo sul nome riservato nella sola guardia | C-181 |
...
| G47 | L'esca non portata ai permessi del documento prima della richiesta | C-189 |
| G48 | Esca ricreata solo per gli ambiti, non per quella generale (codice del dodicesimo giro; oggi non ha un equivalente) | C-189 |
...
| G58 | L'esca che c'era già allineata sul posto invece che ricreata (codice del dodicesimo giro; oggi il suo posto lo prende il G60) | C-192 |
| G59 | L'impronta di accesso senza l'identità del processo | nessuna, detto sopra |
| G60 | L'esca chiesta sempre con lo stesso nome fisso | C-193, e sedici altre |
| G61 | L'esca lasciata nella sottocartella dopo la richiesta | C-193, C-192, C-181, C-180, C-183 |
| G62 | Riservato solo il nome esatto dell'esca, non tutti quelli che cominciano così | C-181 |
| G63 | Nomi riservati confrontati distinguendo maiuscole e minuscole | C-181 |
...
libero il nome, e la copia scriverebbe dove punta. Anche quello si ferma. E un gradino più
su: se fosse la cartella protetta stessa a essere un collegamento verso un'altra cartella dei
caricamenti, che ha un suo indirizzo, il confronto con la sua radice vera passerebbe sempre.
Per questo la cartella attesa si ricava dalla radice vera della cartella dei caricamenti, che
invece può stare dietro un collegamento, com'è normale quando i file stanno su un disco
condiviso. Riga C-183.
...
guardare la destinazione. **Trovato rileggendo la correzione**: se lo stato dell'origine
risultasse vuoto a metà strada, per esempio perché qualcosa lo ha azzerato, il controllo
sarebbe saltato in silenzio. Adesso un'origine che la guardia non conosce ferma lo
spostamento. Riga C-184.
...
### Che cosa ha trovato il nono giro di revisione indipendente

Tre rilievi, accolti tutti e tre: due nel deposito e uno in una prova. Nessun aggiramento nuovo
della catena pubblica, del nonce, della capability o della scadenza, per il terzo giro di
seguito.

**Un ambito misurato altrove tornava valido tornando indietro.** La correzione dell'ottavo giro
legava all'identità della cartella solo l'esito generale. Un ambito provato sull'indirizzo B,
mentre il generale era ancora quello di A, restava conservato; tornati ad A, il generale tornava
valido e l'ambito di B con lui, e diceva `verificata` per un indirizzo di A mai provato. Adesso
ogni ambito porta la propria identità, e si legge solo se è quella della cartella di adesso.
Riga C-187, seconda prova.

**Un deposito chiamato da dentro un altro toglieva le protezioni a quello esterno.** Il
deposito aggancia guardia e dirottamento per il tempo dello spostamento e li sgancia alla fine.
WordPress non registra due volte lo stesso aggancio, quindi un deposito interno li sganciava
per tutti e due, e quello esterno riprendeva senza guardia e senza dirottamento: nella prova
il file è finito davvero nella cartella pubblica dei caricamenti. Adesso un deposito rifiuta di
cominciare mentre un altro è in corso, senza toccare niente, e il segno si toglie in un
`finally` che copre tutto il deposito esterno. Riga C-188.

**La C-181 non arrivava alla guardia.** Il nome riservato si presentava già all'ingresso, e il
rifiuto veniva dal nome previsto: togliendo il controllo nella guardia la riga restava verde.
Il controllo nella guardia c'era e funzionava, ma nessuna prova lo misurava. Adesso una
seconda metà impone il nome dell'esca solo durante lo spostamento, e il G43 la vede rossa.
È la terza prova in due giri che si fermava prima del punto che dichiarava di provare.
...
### Che cosa ha trovato l'undicesimo giro di revisione indipendente

Un rilievo, accolto, dentro la correzione del decimo giro, e giudicato realistico dalla
revisione stessa: basta un server statico con un utente diverso da quello di PHP e una
correzione dei permessi. Nessun aggiramento nuovo della catena pubblica, del nonce, della
capability o della scadenza, per il quinto giro di seguito.

**I permessi del documento non dicono se il server ci arriva.** La correzione del decimo giro
conservava accanto a ogni esito i permessi che WordPress avrebbe dato al documento. Ma il
server arriva a un file solo se attraversa ogni cartella lungo il percorso, e quello lo
decidono i bit di esecuzione delle cartelle. Con la sottocartella del mese a 0744 il documento
era leggibile ma irraggiungibile, e l'ambito risultava negato; portata a 0755, il documento
aveva gli stessi permessi di prima e l'esito restava valido, mentre il file era ormai
scaricabile. Adesso ogni esito conserva un'impronta di proprietario, gruppo e permessi interi
di ogni cartella, dalla cartella dell'esca fino a quella dei caricamenti compresa, e con
...
### Che cosa ha trovato il tredicesimo giro di revisione indipendente

Un rilievo, accolto, ancora dentro la correzione del giro prima, e ancora giudicato
realistico: basta nginx con la memoria dei file aperti che conserva anche gli errori, una
configurazione diffusa, e una richiesta all'esca in un momento in cui mancava. Nessun
aggiramento nuovo della catena pubblica, del nonce, della capability o della scadenza, per il
settimo giro di seguito.

**Un file nuovo con un nome vecchio ha la storia del nome vecchio.** La correzione del
dodicesimo giro ricreava l'esca come file nuovo, ma con lo stesso nome, quindi allo stesso
indirizzo. Un server che ricorda la risposta data a un percorso, come nginx quando conserva
gli errori dei file aperti, poteva ripetere il 404 dato quando l'esca mancava, e la verifica
lo leggeva come un diniego; il documento nuovo, con un nome mai chiesto, non aveva nessun
errore ricordato e veniva servito. È il quarto rilievo di fila con la stessa causa: l'esca
non era un sostituto fedele del documento. Dopo i permessi, l'attraversamento e il file
stesso, qui era il nome.

La correzione toglie anche questa differenza alla radice, e con lei tutta la sottofamiglia
della storia di un percorso: a ogni verifica l'esca nasce con un nome nuovo e imprevedibile,
`prova-accesso-diretto-` più dodici caratteri casuali, nella sottocartella e con l'estensione
del documento, aperta in modo esclusivo e portata ai suoi permessi; si chiede quel nome, e
finita la richiesta il file si toglie, anche quando la richiesta fallisce. Un parametro
casuale nell'indirizzo non sarebbe bastato, e la revisione lo diceva: la memoria dei file
